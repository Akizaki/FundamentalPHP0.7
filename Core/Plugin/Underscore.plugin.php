...
class Underscore implements IteratorAggregate, Countable {
    ...
    static public function __callStatic($name, $args)
    {
        ...
        return count($args) > 1 ? call_user_func_array(array($instance, $name), array_slice($args, 1)) : $instance->$name();
    }
    ...
    {
        foreach ($this->valueOf() as $index => $value)
            if (!$callback($value))
                return false;
        return true;
    }
...
//		return new self(array_filter($this->value(), $callback, ARRAY_FILTER_USE_BOTH));
    ...
    {
        return $this->some(function($item) use($value) {
                    return $item === $value;
                });
    }
    ...
    {
        foreach ($this->valueOf() as $index => $v)
            if ($value === $v)
                return $index;
        return -1;
    }
